export function firstbezierPoints(coords: DoubleHandlers) {
...
export function secondBezierPoints(coords: DoubleHandlers) {
  // P = (1−t)3P1 + 3(1−t)2tP2 +3(1−t)t2P3 + t3P4
  const x0 = 0;
  const x1 = coords.h3.x;
  const x2 = coords.h4.x;
  const x3 = 100;
  const y0 = 0;
  const y1 = coords.h4.y;
  const y2 = coords.h4.y;
  const y3 = 100;
  const values: Point[] = [];
  for (let t = 0; t <= 1; t += 0.05) {
    const x =
